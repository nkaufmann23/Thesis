...
#Only 21 identify as multi-ethnic, so will filter them out
...
#THERE IS ONE NA VALUE DUE TO THERE BEING A BLANK VALUE FOR OBESITY - WHAT DO I DO TO OVERRIDE THE NA?
...
Now, I will create variables for Anxiety and Depression in datasets
```{r}
...
table(CA$Diagnosed_Depression, useNA="always")


AZ <- AZ %>% mutate(Diagnosed_Anxiety = ifelse(Anxiety_Disorder == "1", 1, 0)) %>% mutate(Diagnosed_Depression = ifelse(Depressive_Disorder=="1", 1, 0))
table(AZ$Diagnosed_Anxiety, useNA = "always")
table(AZ$Diagnosed_Depression, useNA="always")
