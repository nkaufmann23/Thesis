...
table(AZ$no_comorbid, useNA="always") #2 people answered "I don't know" to all - will filter them out. There were 4 when I looked in raw data, and 2 had not answered the urban question, so makes sense!
AZ <- AZ %>% filter(no_comorbid !=9)
```

Creating comorbidity score
```{r}
#For CA
CA$comorbid_score = (CA$Heart_Issue=="Yes" | CA$Heart_Issue=="Sí") + (CA$Lung_Issue=="Yes" | CA$Lung_Issue=="Sí") + (CA$Cancer=="Yes" | CA$Cancer=="Sí") + (CA$Hypertension=="Yes" | CA$Hypertension=="Sí") + (CA$Diabetes=="Yes" | CA$Diabetes=="Sí") + (CA$Obesity=="Yes" | CA$Obesity=="Sí") + (CA$Autoimmune_Disorder=="Yes" | CA$Autoimmune_Disorder=="Sí") + (CA$Other_Autoimmune=="Yes" | CA$Other_Autoimmune=="Sí") + (CA$Dementia_Disorder=="Yes" | CA$Dementia_Disorder=="Sí")

table(CA$comorbid_score, useNA = "always")

#For AZ,
AZ$comorbid_score = (AZ$Heart_Issue ==1) + (AZ$Lung_Issue==1)+ (AZ$Cancer==1) + (AZ$Hypertension==1) + (AZ$Diabetes ==1) + (AZ$Obesity ==1) + (AZ$Autoimmune_Disorder==1) + (AZ$Other_Autoimmune==1) + (AZ$Dementia_Disorder==1) 
table(AZ$comorbid_score, useNA="always")
...
#Creating total mental health score for Arizona
AZ <- AZ %>% mutate(PHQ_sum = (sum_anx + sum_dep))
...
#Creating total mental health score for AZ
CA <- CA %>% mutate(PHQ_sum = (sum_anx + sum_dep))
...
```{r}
#Create new data set by merging. #NOT INCLUDING RACE
AZ_new <- AZ %>% select(Likely, location, comorbidity, education, gender, PHQ_dep,PHQ_anx, Age, comorbid_score, PHQ_sum)
CA_new <- CA %>% select(Likely, location, comorbidity, education, gender, PHQ_dep,PHQ_anx, Age, comorbid_score, PHQ_sum)

combined_data <- full_join(AZ_new, CA_new)
str(combined_data)

combined_data_new <- combined_data[complete.cases(combined_data),]
combined_data_new
str(combined_data_new)

age_dist_combined <-  ggplot(combined_data_new, aes(x=Age))+
  geom_histogram(binwidth=1, color="blue", fill="white")+
  xlab("Age")+
  ylab("Count")+
...
#Impact of comorbidity score on likelihood to get vaccinated
logit_crude_comorbid <-glm(Likely ~comorbid_score, data=combined_data_new, family=binomial(link="logit"))
summary(logit_crude_comorbid)
exp(cbind(coef(logit_crude_comorbid), confint(logit_crude_comorbid))) 

logit_crude_comorbid_adjusted <- glm(Likely ~ comorbid_score + location + education + gender + Age, data=combined_data_new, family=binomial(link="logit"))
summary(logit_crude_comorbid_adjusted)
exp(cbind(coef(logit_crude_comorbid_adjusted), confint(logit_crude_comorbid_adjusted))) 

#Impact of comorbidity on likelihood
logit_crude <- glm(Likely ~ comorbidity, data=combined_data_new, family=binomial(link="logit"))
summary(logit_crude)
exp(cbind(coef(logit_crude), confint(logit_crude)))  

logit_1 <- glm(Likely ~ comorbidity + location + education + gender + Age, data=combined_data_new, family=binomial(link = "logit"))
summary(logit_1)
exp(cbind(coef(logit_1), confint(logit_1)))  

#Impact of mental health score on willingness to get vaccinated
logit_mental_crude <- glm(Likely ~ PHQ_sum, data=combined_data_new, family=binomial(link="logit"))
summary(logit_mental_crude)
exp(cbind(coef(logit_mental_crude), confint(logit_mental_crude))) 

logit_mental_adjusted <- glm(Likely ~ PHQ_sum + location + education + gender + Age , data=combined_data_new, family=binomial(link="logit"))
summary(logit_mental_adjusted)
exp(cbind(coef(logit_mental_adjusted), confint(logit_mental_adjusted))) 
  
  
#IMPACT OF ANXIETY ON WILLINGNESS TO GET VAXXED 
logit_2 <- glm(Likely ~ PHQ_anx + location + education + gender + Age, data=combined_data_new, family=binomial(link="logit"))
summary(logit_2)
exp(cbind(coef(logit_2), confint(logit_2)))  


#IMPACT OF DEPRESSION ON WILLINGNESS TO GET VAXXED 
logit_3 <- glm(Likely ~ PHQ_dep + location + education + gender + Age, data=combined_data_new, family=binomial(link="logit"))
summary(logit_3)
exp(cbind(coef(logit_3), confint(logit_3))) 


#SHOULD I FIRST SEE IF COVARIATES ARE ASSOCIATED WITH EXPOSURE / OUTCOME AND MEET DEF OF CONFOUNDER?
#Will need to redo associations with age considering not normal distribution
install.packages("tableone")
library(tableone)
#age, location associated with depression
contvars <- c("Age")
catvars <- c("gender","location","education")
tab1 <- CreateTableOne(vars=c(contvars, catvars), strata="PHQ_dep", data=combined_data_new, factorVars = catvars)
tab1

#do they even need to be stat sig associated? I don't think needs to be stat sig, just associated?
#Age associated with anxiety
tab2<- CreateTableOne(vars=c(contvars, catvars), strata="PHQ_anx", data=combined_data_new, factorVars = catvars)
tab2

#age, location associated with comorbidity
tab3 <- CreateTableOne(vars=c(contvars, catvars), strata="comorbidity", data=combined_data_new, factorVars = catvars)
tab3

#Now, seeing association with outcome. Age/location are associated with outcome
tab4<- CreateTableOne(vars=c(contvars, catvars), strata="Likely", data=combined_data_new, factorVars = catvars)
tab4
